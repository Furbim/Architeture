...
function fetchAgendamentos($pdo, $id_cliente) {
    ...
    WHERE agendamentos.cliente = :id_cliente AND status = 'Agendado'
    ORDER BY agendamentos.data DESC");
    $query->execute(['id_cliente' => $id_cliente]);
    return $query->fetchAll(PDO::FETCH_ASSOC);
}

$agendamentos = fetchAgendamentos($pdo, $id_cliente);
...
<div class="footer_section" style="background: #5a8e94;">
    <div class="container">
        <div class="footer_content">
            <?php if ($temAgendamentos): ?>
                <?php foreach ($agendamentos as $agendamento): 
                    ...
                    <div class='agendamento' id='lista-agendamentos'>
                        ...
                        <p>Serviço: <?= $agendamento['nome_servico'] ?> - R$ <?= number_format($agendamento['valor_servico'], 2, ',', '.') ?></p>
            ...
            <form id="form-agenda" method="post" style="margin-top: -25px !important; <?= $temAgendamentos ? 'display: none;' : '' ?>">
                <div class="footer_form footer-col">
                    <div class="form-group">
                        <input onkeyup="buscarNome()" class="form-control" type="text" name="telefone" id="telefone" value='<?= $telefone ?>' placeholder="Seu Telefone DDD + número" required />
                    </div>
                    <div class="form-group">
                        <input onclick="buscarNome()" class="form-control" type="text" name="nome" id="nome" value="<?= $nome ?>" placeholder="Seu Nome" required />
                    </div>
                    <div class="form-group">
                        <input onchange="mudarFuncionario()" class="form-control" type="date" name="data" id="data" value="<?= $data_atual ?>" required />
                    </div>
                    <div class="form-group">
